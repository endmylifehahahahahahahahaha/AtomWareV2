        ...
        if suc then
            local function InfoNotification(title, text, delay, button_table)
                local suc, res = pcall(function()
                    local frame = GuiLibrary.CreateNotification(title or "AtomWare", text or "Successfully called function", delay or 7, "assets/InfoNotification.png", button_table)
                    return frame
                end)
                return (suc and res)
            end
            InfoNotification("AtomWare x Rivals Loader", "Successfully loaded the script! Please check its credits and support its creator", 3)
        else
            errorNotification("AtomWare x Rivals Loader", "Failure loading the script! Error was sent in the console (type /console)", 3)
        end
...
if not isfile('vape/Libraries/RivalsScriptDecision.txt') or readfile('vape/Libraries/RivalsScriptDecision.txt') == "no" then 
    InfoNotification2("AtomWare x Rivals", "Would you like to load a script made specifically for the game Rivals?", 10000000, interactable_buttons_table)
else
    load()
end