        ...
        Function = function(enabled)
            ...
            if enabled then
				if store.queueType == "halloween_2025_event_pve" then 
					...
					Invisibility.ToggleButton(false)
					return
				end
...
run(function()
    ...
    pcall(function()
        queupath = game:GetService("ReplicatedStorage")
            :WaitForChild("events-@easy-games/lobby:shared/event/lobby-events@getEvents.Events", 5)
        if queupath then
            queupath = queupath:FindFirstChild("joinQueue")
        end
    end)
                ...
                task.spawn(function()
                    while runcheck and AutoFarm.Enabled do
                        ...
                        if ok and state then
                            local matchstats = state.Game and state.Game.matchState
                            ...
                            if matchstats == 3 and queupath then
                                pcall(function() queupath:FireServer(unpack(queueArgs)) end)
                            end
                        end
                        task.wait(1)
                    end
...
run(function()
    ...
    local function findLowestBlock()
        ...
        if lowest == 99999 then
            if entityLibrary.isAlive and lplr.Character and lplr.Character:FindFirstChild("HumanoidRootPart") then
    ...
    WaterAmbient = GuiLibrary.ObjectsThatCanBeSaved.WorldWindow.Api.CreateOptionsButton({
        ...
        Function = function(callback)
            ...
            if callback then
                ...
                if entityLibrary.isAlive and lplr.Character and lplr.Character:FindFirstChild("Humanoid") then
                    lplr.Character.Humanoid:SetStateEnabled(Enum.HumanoidStateType.Swimming, false)
                end
                ...
                waterY = 0
                if entityLibrary.isAlive and lplr.Character and lplr.Character:FindFirstChild("Humanoid") then
                    lplr.Character.Humanoid:SetStateEnabled(Enum.HumanoidStateType.Swimming, true)
                end
            end
        end,
                        ...
                        for _, v in part:GetDescendants() do
                            if v:IsA("TextLabel") then
                                if customMSG and customMSG.Enabled then
                                    v.Text = randomizer(DamageMessages) or v.Text
                                end
...
run(function()
    ...
    VelocityPlus.CreateSlider({
        Name = "Chance",
        Min = 0, Max = 100, Default = 100,
        Function = function(val) VPChance.Value = val end
    })
    VelocityPlus.CreateToggle({
        Name = "Only when targeting",
        Function = function(val) VPTargetCheck.Enabled = val end
    })
end)
...
run(function()
    ...
    local function buildGhost()
        ...
        ghostConn = game:GetService("RunService").PostSimulation:Connect(function()
    ...
    local function buildRangeVisuals()
        ...
        rangeConn = game:GetService("RunService").PostSimulation:Connect(function()
            ...
            local seen = {}
            for _, ent in entityLibrary.entityList do
            ...
            for ent in rangeCircles do if not seen[ent] then destroyRangeCircle(ent) end end
        end)
    end
...
-- VelocityMore (advanced velocity with Normal/Delayed/JumpReset modes)
run(function()
    local VelocityMore
            ...
            if callback then
                ...
                if mode == 'Normal' then
                    ...
                    bedwars.KnockbackUtil.applyKnockback = function(root, mass, dir, knockback, ...)
                        ...
                        local check = (not VMTargetCheck.Enabled) or EntityNearPosition(50)
                        if check then
                ...
                elseif mode == 'Delayed' then
                    ...
                    bedwars.KnockbackUtil.applyKnockback = function(root, mass, dir, knockback, ...)
                        if rand2:NextNumber(0,100) > VMChance.Value then return oldVM(root, mass, dir, knockback, ...) end
                        local check = (not VMTargetCheck.Enabled) or EntityNearPosition(50)
                        if not check then return oldVM(root, mass, dir, knockback, ...) end
                        local char = lplr.Character
                ...
                elseif mode == 'JumpReset' then
                    ...
                    bedwars.KnockbackUtil.applyKnockback = function(root, mass, dir, knockback, ...)
                        ...
                        local check = (not VMTargetCheck.Enabled) or EntityNearPosition(50)
                        local char = lplr.Character
                        if check and entityLibrary.isAlive and char and root == char:FindFirstChild("HumanoidRootPart") then
    ...
    VelocityMore.CreateDropdown({
        ...
        Function = function(val)
            VMMode.Value = val
            updateVisibility()
            if VelocityMore.Enabled then
                VelocityMore.ToggleButton(false)
                VelocityMore.ToggleButton(true)
            end
        end
    })
...
run(function()
    ...
    local AATargets = {Players = {Enabled = true}, Walls = {Enabled = false}, NPCs = {Enabled = false}}
    ...
    local function getAimPos(ent)
        if AAAimPart.Value == 'Closest' then
            ...
            local localPos, mag, part = getMousePosition(), 9e9, nil
            for _, v in cache2[ent.Character] do
    ...
    local aaFuncs = {
        Simple = function(cf, ent, fps)
            ...
            local rawSpeed = AAAimSpeed.Value + (AAStrafeIncrease.Enabled and (game:GetService("UserInputService"):IsKeyDown(Enum.KeyCode.A) or game:GetService("UserInputService"):IsKeyDown(Enum.KeyCode.D)) and 10 or 0)
        ...
        Adaptive = function(cf, ent, fps)
            ...
            local speed = (AAAimSpeed.Value*0.1*prog) + (1-prog) + (AAStrafeIncrease.Enabled and (game:GetService("UserInputService"):IsKeyDown(Enum.KeyCode.A) or game:GetService("UserInputService"):IsKeyDown(Enum.KeyCode.D)) and 10 or 5)
            local alpha = math.clamp((speed / math.max(AASmoothness.Value, 1)) * fps, 0, 1)
            return cf:Lerp(CFrame.lookAt(cf.p, getAimPos(ent) + Vector3.new((rng:NextNumber()-0.5)*AAShake.Value*fps, (rng:NextNumber()-0.5)*AAShake.Value*fps, (rng:NextNumber()-0.5)*AAShake.Value*fps)), alpha)
        end
    }

    local function getAATarget()
        if aaLastTarget then
            local char = lplr.Character
            if not char then return false end
            local localPos = char:FindFirstChild("HumanoidRootPart") and char.HumanoidRootPart.Position
            if not localPos then return false end
            if not aaLastTarget.RootPart or not aaLastTarget.Humanoid or aaLastTarget.Humanoid.Health <= 0 then return false end
            if (localPos - aaLastTarget.RootPart.Position).Magnitude > AADistance.Value then return false end
            return aaLastTarget
        end
        return false
    end
    ...
    local function getAAAttackData()
        ...
        if AAMouse.Enabled and not inputSvc:IsMouseButtonPressed(0) and (tick() - bedwars.SwordController.lastSwing) > 0.15 then return false end
        ...
        if (tick()-aaStarted) > 1 or not aaLastTarget or not aaLastTarget.RootPart or not aaLastTarget.Humanoid or aaLastTarget.Humanoid.Health <= 0 then
            local char = lplr.Character
            local localPos = char and char:FindFirstChild("HumanoidRootPart") and char.HumanoidRootPart.Position
            local ent = getAATarget() or (AAKillauraTarget.Enabled and store.KillauraTarget) or EntityNearPosition(AADistance.Value)

            if ent and AAEnemyPriority.Value ~= 'None' and localPos then
                local validTargets = {}
    ...
    AimAssistPlus = GuiLibrary.ObjectsThatCanBeSaved.CombatWindow.Api.CreateOptionsButton({
        ...
        Function = function(callback)
            if callback then
                table.insert(vapeConnections, game:GetService("RunService").PostSimulation:Connect(function(dt)
                    local ent = getAAAttackData()
                    if ent then
                        local delta = (ent.RootPart.Position - gameCamera.CFrame.p)
                        local angle = math.acos(gameCamera.CFrame.LookVector.Unit:Dot(delta.Unit))
    ...
    AimAssistPlus.CreateDropdown({ Name = "Target Mode", List = {"Distance","Angle","Damage"}, Default = "Angle", Function = function(v) AASort.Value = v end })
...
-- Hit Sound: plays a custom sound on sword hit
run(function()
    local HitSound
    local HitVolume = {Value = 0.5}
    local HitPitch = {Value = 1.0}
    local HitSoundId = {Value = "rbxassetid://9120386446"} -- default: classic hit sound
    local soundObj = nil
    ...
    HitSound = GuiLibrary.ObjectsThatCanBeSaved.RenderWindow.Api.CreateOptionsButton({
        ...
        Function = function(callback)
            if callback then
                ...
                table.insert(vapeConnections, workspace.DescendantAdded:Connect(function(part)
                    if part.Name == "DamageIndicatorPart" and part:IsA("BasePart") then
                        if HitSound.Enabled and soundObj then
                            soundObj:Play()
                        end
                    end
                end))
...
-- Crosshair: draws a custom crosshair on screen
run(function()
    local Crosshair
...
-- Kill Streak Tracker: tracks kills in a session and shows a notification at milestones
run(function()
    local KillStreak
    local KSNotify = {Enabled = true}
    local KSReset = {Enabled = true}
    local killCount = 0
    local deathCount = 0
    ...
    local function updateDisplay()
        if ksLabel then
            ...
            ksLabel.Text = string.format("Kills: %d  Deaths: %d  KDR: %.1f  Time: %02d:%02d",
                killCount, deathCount,
                deathCount == 0 and killCount or killCount/deathCount,
                mins, secs)
        end
    end

    local function buildGui()
        if ksGui then return end
        ksGui = Instance.new("ScreenGui")
        ksGui.Name = "KillStreakGui"
        ksGui.ResetOnSpawn = false
        ksGui.IgnoreGuiInset = true
        ksGui.DisplayOrder = 50
        ksGui.Parent = lplr.PlayerGui
        local frame = Instance.new("Frame")
        frame.BackgroundColor3 = Color3.fromRGB(0,0,0)
        frame.BackgroundTransparency = 0.5
        frame.Size = UDim2.new(0, 340, 0, 28)
        frame.Position = UDim2.new(0.5, -170, 0, 8)
        frame.BorderSizePixel = 0
        Instance.new("UICorner", frame).CornerRadius = UDim.new(0, 6)
        frame.Parent = ksGui
        ksLabel = Instance.new("TextLabel")
        ksLabel.Size = UDim2.new(1, -8, 1, 0)
        ksLabel.Position = UDim2.new(0, 4, 0, 0)
        ksLabel.BackgroundTransparency = 1
        ksLabel.TextColor3 = Color3.fromRGB(255,255,255)
        ksLabel.TextStrokeTransparency = 0.5
        ksLabel.TextScaled = true
        ksLabel.Font = Enum.Font.GothamBold
        ksLabel.Text = "Kills: 0  Deaths: 0  KDR: 0.0  Time: 00:00"
        ksLabel.Parent = frame
    end

    KillStreak = GuiLibrary.ObjectsThatCanBeSaved.UtilityWindow.Api.CreateOptionsButton({
        Name = "KillStreakTracker",
        Function = function(callback)
            if callback then
                killCount = 0
                deathCount = 0
                sessionStart = tick()
                buildGui()
                ksConn = game:GetService("RunService").Heartbeat:Connect(function()
                    if not KillStreak.Enabled then return end
                    updateDisplay()
                end)
                table.insert(vapeConnections, ksConn)
                -- Track kills via damage indicators appearing
                table.insert(vapeConnections, workspace.DescendantAdded:Connect(function(part)
                    if not KillStreak.Enabled then return end
                    if part.Name == "DamageIndicatorPart" and part:IsA("BasePart") then
                        -- Check if it's a kill (health reaches 0 on a target)
                        task.wait(0.05)
                        for _, ent in pairs(entityLibrary.entityList) do
                            if ent.Humanoid and ent.Humanoid.Health <= 0 and ent.Targetable then
                                killCount = killCount + 1
                                if KSNotify.Enabled then
                                    local msg = milestoneMessages[killCount]
                                    if msg then warningNotification("Kill Streak", msg, 3) end
                                end
                                break
                            end
                        end
                ...
                if lplr.Character and lplr.Character:FindFirstChild("Humanoid") then
                    table.insert(vapeConnections, lplr.Character.Humanoid.Died:Connect(function()
                        if not KillStreak.Enabled then return end
                        deathCount = deathCount + 1
                        if KSReset.Enabled then killCount = 0 end
                    end))
                end
                table.insert(vapeConnections, lplr.CharacterAdded:Connect(function(char)
                    local hum = char:WaitForChild("Humanoid", 5)
                    if hum then
                        table.insert(vapeConnections, hum.Died:Connect(function()
                            if not KillStreak.Enabled then return end
                            deathCount = deathCount + 1
                            if KSReset.Enabled then killCount = 0 end
                        end))
                ...
                if ksGui then ksGui:Destroy() ksGui = nil ksLabel = nil end
            end
        end,
        HoverText = "Tracks your kills, deaths, KDR and session time"
    })
    KSNotify = KillStreak.CreateToggle({ Name = "Milestone Notifications", Default = true, Function = function(v) KSNotify.Enabled = v end })
    KSReset  = KillStreak.CreateToggle({ Name = "Reset streak on death",   Default = true, Function = function(v) KSReset.Enabled = v end })
end)

-- Anti Void: teleports you back to your last safe position if you fall into the void
run(function()
    local AntiVoid
    ...
    AntiVoid = GuiLibrary.ObjectsThatCanBeSaved.UtilityWindow.Api.CreateOptionsButton({
        ...
        Function = function(callback)
            if callback then
                avConn = game:GetService("RunService").Heartbeat:Connect(function()
                    if not entityLibrary.isAlive then return end
                    local char = lplr.Character
                    if not char then return end
                    local root = char:FindFirstChild("HumanoidRootPart")
                    ...
                    if pos.Y > AVThreshold.Value then
                        ...
                        if lastSafePos then
                            ...
                            if AVNotify.Enabled then
                                warningNotification("AntiVoid", "Saved from void!", 2)
                            end
                        end
                    end
                end)
...
-- Chat Spam: sends a message in chat on a timer (client-side visible or server fire)
run(function()
    local ChatSpam
    local CSMessage = {Value = "Catware on top"}
    local CSDelay = {Value = 5}
    local CSRunning = false

    ChatSpam = GuiLibrary.ObjectsThatCanBeSaved.FunnyWindow.Api.CreateOptionsButton({
        Name = "ChatSpam",
        Function = function(callback)
            if callback then
                CSRunning = true
                task.spawn(function()
                    while CSRunning and ChatSpam.Enabled do
                        pcall(function()
                            local tcs = game:GetService("TextChatService")
                            if tcs and tcs.TextChannels then
                                local ch = tcs.TextChannels:FindFirstChild("RBXGeneral")
                                if ch then ch:SendAsync(CSMessage.Value) end
                            end
                        end)
                        task.wait(math.max(CSDelay.Value, 1))
                    end
                end)
            else
                CSRunning = false
        ...
        HoverText = "Sends a chat message on a timer"
    })
    ChatSpam.CreateTextBox({ Name = "Message", Default = "Catware on top", Function = function(v) CSMessage.Value = v end })
    ChatSpam.CreateSlider({ Name = "Delay (s)", Min = 1, Max = 60, Default = 5, Function = function(v) CSDelay.Value = v end })
end)

-- Fullbright: sets ambient and outdoor ambient to max brightness
run(function()
    local Fullbright
...
-- ESP Health Bars: draws health bars above players using BillboardGuis
run(function()
    local ESPHealth
            ...
            if callback then
                ...
                espConn = game:GetService("RunService").Heartbeat:Connect(function()
                    for plr, data in pairs(espBillboards) do
                        if not data.hum or not data.hum.Parent then
                            removeBillboard(plr)
                            continue
                        end
