if shared.VapeExecuted then
	...
	GuiLibrary.SaveSettings = function()
		...
		pcall(function() writefile(baseDirectory..profilesDirectory..(shared.CustomSaveVape or game.PlaceId)..".vapeprofiles.txt", httpService:JSONEncode(GuiLibrary.Profiles)) end)
		local WindowTable = {}
		...
		pcall(function() GuiLibrary.Settings["MobileButtons"] = {["Type"] = "MobileButtons", ["Buttons"] = mobileButtonSaving} end)
		pcall(function() WindowTable["GUIKeybind"] = {["Type"] = "GUIKeybind", ["Value"] = GuiLibrary["GUIKeybind"]} end)
		pcall(function() writefile(baseDirectory..profilesDirectory..(GuiLibrary.CurrentProfile == "default" and "" or GuiLibrary.CurrentProfile)..(shared.CustomSaveVape or game.PlaceId)..".vapeprofile.txt", httpService:JSONEncode(GuiLibrary.Settings)) end)
		pcall(function() writefile(baseDirectory..profilesDirectory..(game.GameId).."GUIPositions.vapeprofile.txt", httpService:JSONEncode(WindowTable)) end)
	end
			...
			if (not suc) then 
				if shared.VoidDev then
					task.spawn(function()
						...
						errorNotification("AtomWare | "..tostring(S_Name), debug.traceback(tostring(err)), 10)
					end)
				end
				task.spawn(function()
					repeat task.wait() until errorNotification ~= nil and type(errorNotification) == "function"
					if S_Name ~= "Not Specified" then
						if attemptedRestarts[S_Name] then 
							errorNotification('AtomWare | '..tostring(S_Name), "Restart failed!", 3)
							errorNotification("AtomWare | "..tostring(S_Name), "There was an error with this module. If you can please send the\n VW_Error_Log.json in your workspace to erchodev#0 or discord.gg/atomware", 10)
						else
							if shared.VoidDev then
								errorNotification('AtomWare | '..tostring(S_Name), "There was an error with this module. Attempting restart...", 3)
							end
							attemptedRestarts[S_Name] = true
							local suc2, err2 = pcall(function() func(false) end)
							if suc2 then InfoNotification("AtomWare | "..tostring(S_Name), "Restart successfull!", 3); end
						end
					else
						errorNotification("AtomWare | "..tostring(S_Name), "There was an error with this module. If you can please send the\n VW_Error_Log.json in your workspace to erchodev#0 or discord.gg/atomware", 10)
					end
			...
			if argstablemain["WhitelistRequired"] then
				...
				if shared.vapewhitelist:get(game:GetService("Players").LocalPlayer) == argstablemain["WhitelistRequired"] or shared.vapewhitelist:get(game:GetService("Players").LocalPlayer) > argstablemain["WhitelistRequired"] then 
					...
					task.spawn(function()
						...
						warningNotification("AtomWareWL", "Access granted for "..((argstablemain["Name"] and tostring(argstablemain["Name"])) or "UnknownModule").."["..((argstablemain2["Name"] and tostring(argstablemain2["Name"])) or "Unknown").."Window]", 3)
					end)
				end
